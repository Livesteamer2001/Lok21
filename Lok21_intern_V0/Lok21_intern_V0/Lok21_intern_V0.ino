 ...
 * Die ESP-BoardSW muss unbedingt 2.0.17 sein, sonst geht Servo nicht!!
 * 
 */
#include <SPI.h>
...
#include <movingAvg.h>
#include <MovingAverage.h>       // https://github.com/careyi3/MovingAverage
//#include <pwmWrite.h>             // https://github.com/Dlloydev/ESP32-ESP32S2-AnalogWrite?tab=readme-ov-file
...
// PWM auf ESP32
const uint32_t freq_fahren = 440;          // PWM-Frequenz
const uint32_t freq_bremsen = 200; 
...
unsigned long previousRiWendMillis = millis();
...
int PWMFahren[12];        
int PWMFahren_schnell[12] = {0, 8, 20, 45, 70, 90, 110, 130, 155, 180, 210, 240};  // PWM-Werte für Fahrstufen Schnellgang  Vmax 105
int PWMFahren_langsam[12] = {0, 5, 15, 25, 35, 45, 65, 80, 95, 110, 125, 140};  // PWM-Werte für Fahrstufen Langsamgang  Vmax 52
int PWMBremsen[6] = {0, 80, 150, 220, 240, 254};                            // PWM-Werte für Bremsstufen
...
StructSerTX SerTX;
 ...
 //-----------------------------------------------------------------------------------------------------------        
void setup() {
  // PWM auf Null stellen
  
  // Panto initialisieren, sonst kommt es zu interessanten Effekten...
  ...
  analogWriteFrequency(freq_fahren);
  ...
  if (_radio.hasData()) {
   ...
   //s9Timer.start();
   sensors.begin();
   PCF_SOUND.digitalWrite(SOUND1, LOW);       // Einmal kurz pfeifen als Bestätigung für setup
   delay(30);
   PCF_SOUND.write(0xFF);
}
  ...
  if (PWMMot_soll > PWMMot_ist) {          // Aufschalten
    ...
    if (millis() - prevPWMMotMillis > PWMUPTime) {   //Stufenzeit erreicht
      if (PWMMot_soll > PWMMot_ist) {
        if (AfterNeutral == true) {        // Vermeidung eines Bremsmomentes, wenn bei fahrender Lok von 0 aufgeschaltet wird 
          if (DataTX.kmh > 5) {            
              PWMMot_ist = DataTX.kmh;       
              if (PWMMot_ist > 240) {
                PWMMot_ist = 240;
              }
          }
        ...
        if (PWMMot_soll < PWMMot_ist) {
          if (millis() - prevPWMMotMillis > PWMUPTime) {
            --Fahrstufe_ist;
           if (PWMMot_soll < PWMMot_ist) {
               if ((Fahrstufe_ist > Fahrstufe_soll)) {
...
void _PWM_OUT() { 
  ...
  */
    ...
    if (Fahrstufe_soll == 0) {
      ...
      analogWriteFrequency(freq_bremsen);
      // Richtung berücksichtigen
      if (DataRX.RiWend == 'R') {
